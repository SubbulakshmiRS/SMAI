...
class KNNClassifier:
    ...
    def get_neighbors(self, trainData, test_row, num_neighbors):
        distances = self.manhattan_distance(test_row, trainData[:,1:])
    ...
    def test(self, featuresd, labelsd, TestData):
        trainData = np.c_[ labelsd, featuresd ]
        predictions = np.zeros(len(TestData))
        for index in range(len(TestData)):
            # print(index)
            test_row = TestData[index]
            ...
            # print("len", len(test_row))
            # exit(0)
            output_values = self.get_neighbors(trainData, test_row, self.num_neighbors)
            b = Counter(output_values)
            prediction = b.most_common()[0][0]
            predictions[index] = prediction
        return list(predictions)